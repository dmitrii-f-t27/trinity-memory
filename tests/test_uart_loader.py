...
- Icarus runs the board top fpga/ax7203/tms_uart_loader.v (200 MHz differential
  ...
  garbage and glitches on the line (a port that opens), a host that stops
  mid-frame, the reset button during a transfer, frames sent back to back without
  waiting (backpressure through the receive FIFO, every ack delivered), a FIFO
  overflow, a baud change with its fallback, and the board's own divisor (217, 115200
  baud at 25 MHz). The simulation baud rate is 25 MHz / 16 = 1.5625 Mbaud unless a
  scenario says otherwise, which keeps the runs short; the RTL is the same.
...
class Scenario:
    ...
    def expected_bytes(self) -> int:
        return sum(len(proto.expected_bytes(item)) for item in self.model.out)
    ...
    def wait(self):
        ...
        self.cmds.append(f"t {pending} {new * 12 * self.bit_ps + 3 * self.timeout_ps + 20 * 4096 * CLK_PS}")
    ...
    def glitch(self, bits: float):
        ...
        if bits < 0.4:
            ...
            low = int(bits - 0.7) + 1          # samples at about 0.7, 1.7, ... bit periods are low
            byte = 0
            for k in range(8):
                if k + 1 >= low:
                    byte |= 1 << k
...
def parse_capture(path: Path):
    ...
    return received, markers, gave_up
...
def scenario_transfer(rng, skew, bench="top"):
    """Randomized lengths and addresses, each chunk loaded, acknowledged and read back."""
    s = Scenario(f"transfer_{bench}_{skew:+.2f}", bench=bench, skew=skew,
...
def scenario_pipelined(rng):
    ...
    burst += proto.status_frame(7) + proto.read_frame(8, 0x100, 64)
...
def scenario_board_rate(skew):
    ...
    s = Scenario(f"board_rate_{skew:+.2f}", baud_div=217, timeout=1_250_000, probation=75_000_000, skew=skew)
...
def build_benches(work: Path, configs):
    ...
    for bench, params in configs:
        ...
        vvp = work / f"{bench}_{params['BAUD_DIV']}_{params['TIMEOUT_CLOCKS']}.vvp"
...
class LoaderSimulation(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.scenarios = {s.name: s for s in [
            scenario_transfer(rng, 0.03), scenario_transfer(rng, -0.03, bench="ports"), scenario_faults(rng, -0.03),
            scenario_pipelined(rng), scenario_overflow(rng), scenario_baud(rng),
            scenario_board_rate(0.02), scenario_board_rate(-0.02)]}
        ...
        for name, (proc, capture) in results.items():
            received, markers, gave_up = parse_capture(capture) if capture.exists() else ([], {}, [])
            cls.results[name] = {"proc": proc, "received": received, "markers": markers, "gave_up": gave_up,
                                 "events": decode(received)}
        cls.write_summary()
    ...
    def assert_as_predicted(self, name):
        ...
        for i, (g, w) in enumerate(zip(got, want)):
            ...
            if g.kind == "line":
                ...
                self.fail(f"{name}: garbage from the device at event {i}: {g.as_dict()}")
        return r

    def test_randomized_transfers_read_back_identically(self):
        for name in ("transfer_top_+0.03", "transfer_ports_-0.03"):
    ...
    def test_backpressure_on_the_ports(self):
        r = self.completed("transfer_ports_-0.03")
        ...
        s = self.scenarios["transfer_ports_-0.03"]
    ...
    def test_faults_are_detected_and_recovered(self):
        r = self.assert_as_predicted("faults_-0.03")
        ...
        self.assertEqual(status["rx_false_starts"], 2)
