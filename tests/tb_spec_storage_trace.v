...
module tb_spec_storage_trace;
    ...
    wire [9:0] out_trits;
    generate if (DENSE5 != 0) begin : dense
        ternary_dense5_stream_t27 #(.TRIT_COUNT(TRIT_COUNT)) dut (
            .clk(clk), .rst(rst), .start(start), .busy(busy), .load_en(load_en),
            .load_addr(load_addr_full[ADDR_WIDTH-1:0]), .load_code(load_code_full[7:0]),
        ...
        ternary_baseline5_stream_t27 #(.TRIT_COUNT(TRIT_COUNT)) dut (
            .clk(clk), .rst(rst), .start(start), .busy(busy), .load_en(load_en),
            .load_addr(load_addr_full[ADDR_WIDTH-1:0]), .load_code(load_code_full),
            .load_ready(load_ready), .out_valid(out_valid), .out_last(out_last),
            .out_code_valid(out_code_valid), .out_lane_mask(out_lane_mask), .out_trits(out_trits)
        );
    end endgenerate
