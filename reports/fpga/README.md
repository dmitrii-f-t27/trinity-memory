...
Status: the stream-compute trace suites of the merged repository (27 vectors,
288 cycles: 18 dot, 9 storage including backpressure holds) run on the device in
both clock variants and match the reference; the conformance lab carries
`device_evidence: fpga` for them. The 7 `join_trace` vectors are replayed in
Icarus only (they are not part of the player).
