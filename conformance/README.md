...
`path:line` ranges; `UNKNOWN` is reserved for code not found at the pin, and no current
vector uses it), `note`, and,
...
| `safetensors` | `safetensors_hex`, `tensor`, `file_size`, `data_offsets` (every entry's pair), `expect` {`find`, `dtype`, `shape`, `begin`, `end`, `dtype_bits`, `has_next`, `next_begin`, `has_prev`, `prev_end`, `check`} | a safetensors header in a file of `file_size` bytes: lookup, dtype width and the extent rules (numel × width; the tensor must tile with its neighbours: begin at `prev_end`, the greatest end of a nonzero-size tensor that begins below it, or the start of the data section when `has_prev` is false, and end at `next_begin`, the least begin of a nonzero-size tensor at or above it, or at `file_size` when `has_next` is false). When an entry's absolute offset does not fit in 64 bits, `find` is the `extent` status, `check` repeats it and the range fields are 0 |
