...
# A runtime that fails to build is reported and skipped; the script fails only
# when none builds.
set -u
cd "$(dirname "$0")/.."
out=build/replay
...
build() {
    ...
    cmake --build "$src/build" --target ggml -j 4 > "$out/$name-build.log" 2>&1 || { tail -40 "$out/$name-build.log" >&2; return 1; }
    lib="$src/build/bin"
