...
def tcp_session(limits, backend=None):
    if backend is None:
        ...
        return
    # An fpga vector: the device double of tests/fake_fpga_device.py on a pseudo-terminal.
    with spec_bridge_replay.fpga_double(backend) as (path, options):
...
def run_lab(rtl, seed):
    ...
    report = {"schema": "trinity.conformance-lab.v1", "passed": all(section.get("passed") for section in sections.values()),
              ...
              "device_evidence": (sections.get("stream") or {}).get("device")}
    return report
