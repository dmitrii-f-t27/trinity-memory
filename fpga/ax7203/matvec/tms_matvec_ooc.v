...
// synthesis and place-and-route estimates only (tools/fpga-matvec-ooc.sh); never loaded on the
...
// (go, tag, a, b) plus done and busy are XOR-reduced into one registered output pin, so nothing
...
module tms_matvec_ooc (
    ...
    // act_bank, act_data, line_idle: 1 + 4 * 32 + 1 + 128 + 1 + 2 * 32 + 64 + 1 = 388 bits.
    reg [387:0] sr = 388'd0;
    always @(posedge clk) sr <= {sr[386:0], din};
    wire line_go, done, busy;
    ...
    always @(posedge clk) dout <= ^{line_go, line_tag, line_a, line_b, done, busy};
endmodule
