...
## Версия 0.2: пять направлений вместе
...
между компонентами. [Отчёт](reports/stack.html) нужно скачать и открыть локально;
...
Для этих команд нужен Icarus Verilog. Без него доступен программный запуск:
`python3 -m trinity_memory edge-demo`. Wheel включает исходники RTL; сам
симулятор устанавливается отдельно.

Это программный/RTL-демонстратор. HTTP-сервер вычисляет в Python, а RTL отдельно
