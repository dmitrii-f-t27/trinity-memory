...
**Версия 0.4 добавляет [t27 Ternary Check](#t27-ternary-check-версия-04)**, побитовую
...
те же масштабы, бит в бит?** Каждое решение принимает исполняемый t27
...
2. Упакованные триты BitNet нельзя пересчитать из опубликованных весов bf16:
   ...
   ([подробнее](docs/ternary-check.md#results)).
3. В хвостах I2_S остаются лишние байты: после масштаба f32 — 28 ненулевых байт во
   всех 210 тензорах I2_S, равных байтам более раннего тензора на том же смещении
...
Только хранение и целочисленная арифметика: три тензора, ни одна модель не запускалась.
