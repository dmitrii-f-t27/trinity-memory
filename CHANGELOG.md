...
- `t27/live.t27` (issues #48-#51): `tlv_walk` reads a GGUF header as each pinned
  runtime's `gguf.cpp` does (magic, versions and byte order, keys, alignment, names,
  shapes, type ids, whole-block rows, offsets in record order) plus the loader's
  file-end rule, and reports per record the layout that fills a record's place when the
  declared one does not (PrismML-Eng/llama.cpp#167); `tlv_hadamard` restates the PrismML
  fork's `prism.hadamard.*` rules, grouped GDN heads included; `tlv_runtime` gives each
  runtime's verdict (accepts, refuses, ignores a declared rotation); `tlv_file_verdict`.
  Status classes -63 to -84 in `specs/formats/OWNERS.md`; native harness
  `tests/native_live.c`.
- `t27/runtimes.t27`, generated by `tools/generate-runtime-tables.py` from
  `specs/runtimes/*.json`: ggml type ids, block sizes, byte-count rules and architecture
  names of ggml-org/llama.cpp, the PrismML fork and bitnet.cpp at their pinned commits;
  the weekly drift check covers their files.
- `trinity_memory/live.py`: discovery through the Hub API (names, and file names among
  the most downloaded GGUF repositories), header reads by HTTP range requests
  (anonymous, throttled, bounded), byte-identical files read once, a validated header
  cache, `--recheck` offline, `--replay` against the real readers built by
  `tools/live-replay.sh` from `tests/upstream/gguf_replay.c`; workflow
  `ternary-check-live.yml` (weekly and on demand); `docs/live.md`.
