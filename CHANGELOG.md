...
- `t27/live.t27` (issues #48-#51): `tlv_walk` reads a GGUF header as each pinned
  runtime's `gguf.cpp` does (magic, versions and byte order, counts, keys with the
  reader's string and array limits, alignment, names, shapes, type ids, whole-block rows,
  the data section inside the file, offsets in record order, in its own wrapping
  arithmetic), bitnet.cpp's differences included, plus the loader's file-end rule, and
  reports per record the layout that fills a record's place when the declared one does
  not (PrismML-Eng/llama.cpp#167); `tlv_model` gives each runtime's verdict on a model
  (one file, or the parts of a split model read together: split keys, part names,
  names repeated across parts, `split.tensors.count`), in the loader's order (accepts,
  refuses, ignores a declared rotation, or no verdict); `tlv_hadamard` restates the
  PrismML fork's `prism.hadamard.*` rules over every part, grouped GDN heads included;
  `tlv_native` picks the runtime a model is written for (TL2 files included);
  `tlv_file_verdict`. Work limits bound what a crafted header can cost. Status classes
  -63 to -87 in `specs/formats/OWNERS.md`; native harness `tests/native_live.c`.
- `t27/runtimes.t27`, generated by `tools/generate-runtime-tables.py` from
  `specs/runtimes/*.json`: ggml type ids, block sizes, byte-count rules, the architecture
  names each model mapping turns into a model class and those refused as a main model
  (clip), for ggml-org/llama.cpp, the PrismML fork and bitnet.cpp at their pinned
  commits; the weekly drift check covers their files.
- `trinity_memory/live.py`: discovery through the Hub API (names, and file names among
  the most downloaded GGUF repositories; failures recorded), split models grouped by
  their part names, header reads by HTTP range requests (anonymous, throttled, bounded,
  retried also when a body is cut short, Content-Range checked), byte-identical models
  read once, a header cache keyed by LFS SHA-256 and checked by header SHA-256,
  `--recheck` offline, `--replay` against the real readers built by
  `tools/live-replay.sh` from `tests/upstream/gguf_replay.c` (crashes and timeouts
  counted apart, the replayed runtimes recorded); report schema
  `trinity.ternary-check-live.v2`; workflow `ternary-check-live.yml` (weekly and on
  demand, counts only in its log); `docs/live.md`.
